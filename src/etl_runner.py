...
class ETLPipeline:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            handle_pipeline_error(
                e, {"step": "pipeline", "error_type": type(e).__name__}
            )
            sys.exit(1)
        finally:
    ...
    def _run_full_pipeline(self) -> None:
        ...
        self.loader.save_to_parquet(
            transformed_df, partition_cols=["watch_month", "watch_day"]
        )
    ...
    def _run_batch_pipeline(self) -> None:
        ...
        for batch in self.extractor.read_csv_in_batches():
            ...
            self.loader.save_batch_to_parquet(
                transformed_batch, partition_cols=["watch_month", "watch_day"]
            )
    ...
    def _load_to_snowflake(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            handle_pipeline_error(
                e, {"step": "snowflake_load", "error_type": type(e).__name__}
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Netflix-style behavioral data ETL pipeline"
    )
    parser.add_argument("--input", type=str, help="Path to input CSV file")
    parser.add_argument("--output", type=str, help="Path to output Parquet file")
    parser.add_argument("--batch", action="store_true", help="Process data in batches")
    parser.add_argument(
        "--snowflake", action="store_true", help="Load data to Snowflake"
    )
